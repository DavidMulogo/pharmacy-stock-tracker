...
type ExistingBatch = Pick<Database["public"]["Tables"]["inventory_batches"]["Row"], "product_id" | "batch_number" | "expiry_date">;
...
function getBatchKey(batch: ExistingBatch) {
  return `${batch.product_id}::${batch.batch_number.trim().toLowerCase()}::${batch.expiry_date}`;
}

function validateRow(row: Record<string, unknown>, index: number, productsByName: Map<string, ProductRow>) {
  const errors: string[] = [];
...
export async function POST(request: Request) {
  try {
    ...
    const validated = rows.map((row, index) => validateRow(row, index + 2, productsByName));
    ...
    const existingResult = await supabase.from("inventory_batches").select("product_id, batch_number, expiry_date");
