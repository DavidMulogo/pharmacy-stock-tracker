...
function validateRow(row: Record<string, unknown>, index: number) {
  ...
  const reorderLevel = toOptionalNumber(row.reorder_level);
  ...
  if (reorderLevel !== null && (!Number.isInteger(reorderLevel) || reorderLevel < 0)) errors.push("Reorder level must be a whole number zero or greater.");

  for (const column of requiredColumns) {
    if (column === "default_unit_price" || column === "default_pack_price" || column === "reorder_level") continue;
