...
const roleOptions: PharmacyUserRole[] = ["OWNER", "IN_CHARGE", "PHARMACIST", "TECHNICIAN"];
...
function roleBadge(role: PharmacyUserRole) {
  ...
  if (role === "IN_CHARGE") return "border-violet-200 bg-violet-100 text-violet-800";
  if (role === "PHARMACIST") return "border-blue-200 bg-blue-100 text-blue-800";
  return "border-slate-200 bg-slate-100 text-slate-700";
}

export function StaffManager({ currentUserId, currentRole, initialStaff }: { currentUserId: string; currentRole: PharmacyUserRole; initialStaff: PharmacyUser[] }) {
  ...
  const resettableStaff = isOwner
    ? staff.filter((user) => user.role !== "OWNER")
    : staff.filter((user) => user.role === "PHARMACIST" || user.role === "TECHNICIAN");
  ...
  return (
    <div className="grid gap-5">
      {isOwner ? <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          {filteredStaff.map((user) => (
            <article key={user.id} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                {isOwner ? <div className="grid grid-cols-2 gap-2 sm:flex">
