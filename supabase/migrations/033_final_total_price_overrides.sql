alter table public.sales
  add column if not exists override_total numeric(12, 2) null;

update public.sales
set override_total = round(quantity_entered * override_price, 2)
where override_price is not null and override_total is null;

alter table public.sales
  drop constraint if exists sales_override_total_check,
  add constraint sales_override_total_check check (override_total is null or override_total >= 0),
  drop column if exists total_sale,
  add column total_sale numeric(12, 2) generated always as (
    coalesce(override_total, quantity_entered * effective_price)
  ) stored,
  drop column if exists override_flag,
  add column override_flag text generated always as (
    case
      when override_total is not null
        or (final_selling_price is not null and final_selling_price <> default_price)
      then 'OVERRIDDEN'
      else 'NORMAL'
    end
  ) stored,
  add constraint sales_override_flag_check check (override_flag in ('NORMAL', 'OVERRIDDEN'));
...
declare
 ...
 override_total numeric;override_price numeric;default_price numeric;effective_price numeric;v_sale_id uuid;sale_total numeric;total numeric:=0;available integer;
...
begin
 ...
 for item in select value from jsonb_array_elements(p_items) loop
  ...
  begin
   ...
   override_total:=case
    when nullif(btrim(item->>'override_total'),'') is not null then(item->>'override_total')::numeric
    when nullif(btrim(item->>'override_price'),'') is not null then(item->>'override_price')::numeric*qty
    else null
   end;
  ...
  if qty<=0 or sell_type not in('UNIT','PACK') or(override_total is not null and override_total<0) then raise exception 'Cart item % has an invalid quantity, sell type, or final total.',line using errcode='22023';end if;
  ...
  effective_price:=case when override_total is null then default_price else round(override_total/qty,2) end;
  override_price:=case when override_total is null then null else effective_price end;
  ...
  insert into public.sales(pharmacy_id,transaction_id,line_number,product_id,sell_type,quantity_entered,units_sold,quantity_sold,default_price,override_price,override_total,effective_price,final_selling_price,created_at)
   values(p_pharmacy_id,tx,line,v_product_id,sell_type,qty,units,units,default_price,override_price,override_total,effective_price,override_price,tx_time) returning id,total_sale into v_sale_id,sale_total;
