alter table public.products
  ...
  alter column default_pack_price drop not null;

alter table public.products
  drop constraint if exists products_default_unit_price_check,
  add constraint products_default_unit_price_check check (default_unit_price is null or default_unit_price >= 0),
  drop constraint if exists products_default_pack_price_check,
  add constraint products_default_pack_price_check check (default_pack_price is null or default_pack_price >= 0),
  drop constraint if exists products_at_least_one_default_price_check,
  add constraint products_at_least_one_default_price_check check (
    default_unit_price is not null or default_pack_price is not null
  );
...
select
  ...
  p.pack_type,
  p.units_per_pack,
  coalesce(p.default_selling_price, p.default_unit_price, p.default_pack_price / p.units_per_pack) as default_selling_price,
  p.selling_mode,
  coalesce(p.default_unit_price, p.default_pack_price / p.units_per_pack) as default_unit_price,
